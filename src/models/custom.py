...
class CustomAIInterface(AIInterface):
    """自定义 AI API 接口实现"""
    ...
    def _validate_config(self) -> None:
        ...
        Raises:
            ValueError: 配置无效时抛出
        """
        required_keys = ["api_key", "model", "api_endpoint"]
        ...
        if not isinstance(self.config["api_endpoint"], str):
            raise ValueError("api_endpoint must be a string")
            
        logger.info("Custom AI configuration validated successfully")
    ...
    def _make_api_request(self, endpoint: str, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code != 200:
                error_msg = f"API request failed: {response.text}, {response.status_code}"
